...
public:
    ...
    /*!
     * Sets the \a criterion and \a value for the search.
     */
    void setQuery(Criterion criterion, const QString &value);

    /*!
     * Sets the \a criterion and \a value for the search with \a match.
     * \a criterion the query criterion to compare with
     * \a value the value to match against
     * \a match how to match the given value
     * \since 4.5
     */
    void setQuery(Criterion criterion, const QString &value, Match match);

    /*!
     * Sets a \a limit on how many results will be returned by this search job.
     ...
     * search.
     * \a limit the limit to set
     * \note this needs to be called before calling setQuery() to have an effect.
